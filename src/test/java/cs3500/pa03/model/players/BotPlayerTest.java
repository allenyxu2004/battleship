...
class BotPlayerTest {
   ...
   */
  @BeforeEach
  public void initBotPlayerTest() {
    this.random = new Random(2);
    this.botPlayer = new BotPlayer(new BoardsState(), this.random);
  ...
  @Test
  public void name() {
    assertEquals("khvitri", this.botPlayer.name());
  }
  ...
  public void setup() {
    List<Ship> expectedShips = new ArrayList<>(
        Arrays.asList(this.mockCarrier, this.mockBattleship, this.mockDestroyer,
            this.mockSubmarine));
    assertEquals(expectedShips, this.botPlayer.setup(6, 6, this.mockSpecifications));
  }


  /**
   * Test for bot player takeShots() method
   */
  @Test
  public void takeShots() {
    this.setup();
    Coord shot1 = new Coord(0, 5);
    Coord shot2 = new Coord(2, 0);
    Coord shot3 = new Coord(2, 3);
    Coord shot4 = new Coord(4, 3);
    List<Coord> expectedShots = new ArrayList<>(Arrays.asList(shot1, shot2, shot3, shot4));
    assertEquals(expectedShots, this.botPlayer.takeShots());
    this.botPlayer.takeShots();
    this.botPlayer.takeShots();
    this.botPlayer.takeShots();
    this.botPlayer.takeShots();
    this.botPlayer.takeShots();
    ...
    List<Coord> emptyShots = new ArrayList<>();
    assertEquals(emptyShots, this.botPlayer.takeShots());
  }
  ...
  public void reportDamage() {
    ...
    assertEquals(shotsFired1, this.botPlayer.reportDamage(shotsFired1));

    List<Coord> shotsFired2 =
        new ArrayList<>(Arrays.asList(new Coord(0, 4), new Coord(0, 5)));

    List<Coord> missAllShots = new ArrayList<Coord>();
    assertEquals(missAllShots, this.botPlayer.reportDamage(shotsFired2));
  }
  ...
  public void successfulHits() {
    this.botPlayer.successfulHits(new ArrayList<Coord>(Arrays.asList(new Coord(0, 0))));
  }
  ...
  @Test
  public void endGame() {
    this.botPlayer.endGame(new GameResult(), "nothing");
  }
}